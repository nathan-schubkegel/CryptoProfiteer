  ...
  public class DataService : IDataService
  {
    ...
    public void ImportTransactions(IEnumerable<PersistedTransaction> importedTransactions)
    {
      lock (_lock)
      {
        ...
        var autoGeneratedTaxAssociations = BuildFuturesGainsTaxAssociations(newOrders);
        var newTaxAssociations = ImportTaxAssociations(autoGeneratedTaxAssociations, newOrders);
    ...
    private (Dictionary<string, Order> newOrders, HashSet<string> newCoinTypes) BuildOrdersAndCoinTypes(
    ...
    private List<PersistedTaxAssociation> BuildFuturesGainsTaxAssociations(Dictionary<string, Order> newOrders)
    {
      var autoGeneratedTaxAssociations = new List<PersistedTaxAssociation>();
      foreach (var order in newOrders.Values)
      {
        if (order.TransactionType == TransactionType.FuturesPnl &&
            order.ReceivedCoinCount > 0m &&
            order.PaymentCoinCount == 0m)
        {
          autoGeneratedTaxAssociations.Add(new PersistedTaxAssociation
          {
            SaleOrderId = order.Id,
            Purchases = new List<PersistedTaxAssociationPurchase>
            {
              new PersistedTaxAssociationPurchase
              {
                OrderId = order.Id,
                ContributingCoinCount = 0m,
              }
            }
          });
        }
      }
      return autoGeneratedTaxAssociations;
    }
