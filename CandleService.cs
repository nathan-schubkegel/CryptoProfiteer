...
namespace CryptoProfiteer
{
  ...
  public class CandleService : ICandleService
  {
    ...
    public Task<PersistedCandleRange> TryGetCandleRangeAsync(
      PersistedCandleRangeId id,
      ...
      Task.Run(async () =>
      {
        try
        {
          ...
          if (data == null)
          {
            if (id.Exchange != CryptoExchange.CoinbasePro)
              throw new Exception("no can do, hoss; coinbase pro only for now.");

            string responseBody = null;
            var request = new HttpRequestMessage(
              HttpMethod.Get,
              $"https://api.exchange.coinbase.com/products/{id.CoinType}-USD/candles"
            ...
            await _httpClientSingleton.UseAsync(
              $"fetching {id.Granularity} candles for {id.CoinType} at {id.StartTime}",
              stoppingToken,
              async http =>
              {
                ...
                if (!response.IsSuccessStatusCode)
                {
                  throw new HttpRequestException(
                    $"coinbase pro api for {id.Granularity} candles for {id.CoinType} at={id.StartTime} returned {response.StatusCode}: {responseBody}"
                  );
                }
              }
            );
            ...
            else
            {
              ...
              */
              foreach (JArray candleArray in jArray)
              {
                ...
                var dataIndex =
                  (int)Math.Round((candleTime - id.StartTime).TotalSeconds, MidpointRounding.AwayFromZero)
                  / (int)id.Granularity;
                if (dataIndex > id.Count)
                {
                  throw new Exception(
                    $"Received candle data for {candleTime} but that's outside the requested range {id.StartTime} to {id.EndTime}"
                  );
                }
          ...
          return new PersistedCandleRange { Id = id, Candles = data };
        }
