  ...
  public class TaxAssociationSale
  {
    ...
    public TaxAssociationSale(TaxAssociation association, Order order)
    {
      ...
      if (order.TransactionType != TransactionType.Trade && order.TransactionType != TransactionType.FuturesPnl)
        throw new Exception("Tax association Sale data refers to order that is not a trade or a FuturesPnl");
      if (order.PaymentCoinType == "USD")
        throw new Exception(
          "Tax association sale data refers to order that paid USD (nope. if USD is involved in a sale, it's gotta be received USD)"
        );
    }
