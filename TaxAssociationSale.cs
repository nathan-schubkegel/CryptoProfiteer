...
namespace CryptoProfiteer
{
  public class TaxAssociationSale
  {
    ...
    public TaxAssociationSale(TaxAssociation association, Order order)
    {
      ...
      if (order.TransactionType != TransactionType.Trade && order.TransactionType != TransactionType.FuturesPnl) throw new Exception("Tax association Sale data refers to order that is not a trade or a FuturesPnl");
