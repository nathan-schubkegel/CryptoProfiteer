...
namespace CryptoProfiteer
{
  ...
  public class HistoricalCoinPriceService : BackgroundService, IHistoricalCoinPriceService
  {
    ...
    private Dictionary<string, HashSet<CryptoExchange>> _coinTypesSupportedByWhichExchanges =
      new Dictionary<string, HashSet<CryptoExchange>>();

    public IEnumerable<string> CoinbaseCoinTypes =>
      _coinTypesSupportedByWhichExchanges.Where(x => x.Value.Contains(CryptoExchange.Coinbase)).Select(x => x.Key);

    // null PriceUsd means all exchanges have no data for the coin at that time
    private IReadOnlyDictionary<
      (string CoinType, DateTime Time),
      (CryptoExchange Exchange, Decimal? PriceUsd)
    > _historicalPrices =
      new Dictionary<(string CoinType, DateTime Time), (CryptoExchange Exchange, Decimal? PriceUsd)>();

    private ImmutableHashSet<(string CoinType, DateTime Time)> _neededPrices = ImmutableHashSet<(
      string CoinType,
      DateTime Time
    )>.Empty;

    private ImmutableHashSet<(string CoinType, DateTime Time)> _exchangeFailedPrices = ImmutableHashSet<(
      string CoinType,
      DateTime Time
    )>.Empty;

    public HistoricalCoinPriceService(
      IServiceProvider serviceProvider,
      ILogger<HistoricalCoinPriceService> logger,
    ...
    private void ReplaceHistoricalPrices(
      Action<Dictionary<(string CoinType, DateTime Time), (CryptoExchange Exchange, Decimal? PriceUsd)>> mutator
    )
    {
    ...
    public Decimal? ToUsd(Decimal coinCount, string coinType, DateTime time)
    {
      ...
      {
        if (coinType == coinTypeToLog)
          _logger.LogInformation($"Declining ToUsd({coinTypeToLog}, {time}) because time is in the future");
        return null;
      }
        ...
        if (historicalPrice.PriceUsd == null)
        {
          ...
          if (coinType == coinTypeToLog)
            _logger.LogInformation(
              $"Declining ToUsd({coinTypeToLog}, {time}) because the exchanges have no data for the requested time"
            );
          return null;
        }

        if (coinType == coinTypeToLog)
          _logger.LogInformation($"Successfully returning stored ToUsd({coinTypeToLog}, {time})");
      ...
      if (!_coinTypesSupportedByWhichExchanges.ContainsKey(coinType))
      {
        if (coinType == coinTypeToLog)
          _logger.LogInformation(
            $"Declining ToUsd({coinTypeToLog}, {time}) because _coinTypesSupportedByWhichExchanges doesn't have {coinTypeToLog}"
          );
        return null;
      }
      ...
      if (coinType == coinTypeToLog)
        _logger.LogInformation(
          $"Declining ToUsd({coinTypeToLog}, {time}) because that coin+time hasn't been fetched yet; queuing..."
        );
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
      ...
      try
      {
        ...
        await _httpClientSingleton.UseAsync(
          "fetching coinbase trading pairs (for historical price service)",
          stoppingToken,
          async http =>
          {
            ...
            if (!response.IsSuccessStatusCode)
            {
              throw new HttpRequestException(
                $"coinbase pro api for all known trading pairs returned {response.StatusCode}: {responseBody}"
              );
            }

            _logger.LogTrace(
              "HistoricalCoinPriceService processed request for coinbase pro trading pairs, producing this data: {0}",
              responseBody
            );
      ...
      try
      {
        ...
        request.Headers.Add("User-Agent", HttpClientSingleton.UserAgent);
        await _httpClientSingleton.UseAsync(
          "fetching kucoin trading pairs (for historical price service)",
          stoppingToken,
          async http =>
          {
            var response = await http.SendAsync(request, stoppingToken);
            ...
            {
              throw new HttpRequestException(
                $"kucoin api for all known trading pairs returned {response.StatusCode}: {responseBody}"
              );
            }

            _logger.LogTrace(
              "HistoricalCoinPriceService processed request for kucoin trading pairs, producing this data: {0}",
              responseBody
            );
        ...
        try
        {
          ...
          foreach (var (coinType, time) in neededPrices)
          {
            ...
            catch (Exception ex)
            {
              _logger.LogError(
                ex,
                $"Ignoring unhandled exception in HistoricalCoinPriceService neededPrices worker loop"
              );
            }
            finally
            {
    ...
    private async Task<Decimal?> FetchHistoricalPriceFromExchanges(
      string coinType,
      DateTime time,
    ...
    )
    {
      ...
      {
        if (
          supportedExchanges.Contains(CryptoExchange.CoinbasePro)
          || supportedExchanges.Contains(CryptoExchange.Coinbase)
        )
        {
      ...
      {
        _logger.LogInformation(
          $"Failed to fetch price of {coinType} due to exception; will not try again until the application has restarted."
        );
      ...
      _logger.LogInformation(
        $"determined pricePerCoinUsd = {(newPrice?.ToString() ?? "null")} for {coinType} at {time} from {exchange}"
      );

      return newPrice;
    }

    private async Task<Decimal?> FetchCoinbaseHistoricalPrice(
      string coinType,
      DateTime time,
      CancellationToken stoppingToken
    )
    {
      ...
      var request = new HttpRequestMessage(
        HttpMethod.Get,
        $"https://api.exchange.coinbase.com/products/{coinType}-USD/candles"
          + $"?granularity=60"
      ...
      request.Headers.Add("User-Agent", HttpClientSingleton.UserAgent);
      await _httpClientSingleton.UseAsync(
        $"fetching coinbase historical price of {coinType} at {time}",
        stoppingToken,
        async http =>
        {
          var response = await http.SendAsync(request, stoppingToken);
          ...
          {
            throw new HttpRequestException(
              $"coinbase pro api for {coinType} candle at={time.ToString("o")} returned {response.StatusCode}: {responseBody}"
            );
          }

          _logger.LogTrace(
            "HistoricalCoinPriceService processed coinbase request for {0} at {1} from {2}, producing this data: {3}",
            coinType,
            time,
          ...
          else
          {
            ...
            */
            ...
            if (jArray.Count == 0)
              throw new Exception($"No sale history found for {coinType} at {time} on {CryptoExchange.Coinbase}");
    ...
    private async Task<Decimal?> FetchKucoinHistoricalPrice(
      string coinType,
      DateTime time,
      CancellationToken stoppingToken
    )
    {
      ...
      request.Headers.Add("User-Agent", HttpClientSingleton.UserAgent);
      await _httpClientSingleton.UseAsync(
        $"fetching kucoin historical price of {coinType} at {time}",
        stoppingToken,
        async http =>
        {
          var response = await http.SendAsync(request, stoppingToken);
          ...
          {
            throw new HttpRequestException(
              $"kucoin api for {coinType} candle at={time.ToString("o")} returned {response.StatusCode}: {responseBody}"
            );
          }

          _logger.LogTrace(
            "HistoricalCoinPriceService processed kucoin request for {0} at {1} from {2}, producing this data: {3}",
            coinType,
            time,
          ...
          else
          {
            ...
            */
            ...
            TimeSpan closestDifference = DateTime.MaxValue.Subtract(DateTime.MinValue);
            if (data.Count == 0)
              throw new Exception($"No sale history found for {coinType} at {time} on {CryptoExchange.Kucoin}");
    ...
    private async Task<Decimal?> FetchInferredHistoricalPrice(
      string coinType,
      DateTime time,
      CancellationToken stoppingToken
    )
    {
      ...
      foreach (
        var x in dataService.Transactions.Values.Where(x =>
          x.TransactionType == TransactionType.Trade
          && x.Time.ChopSecondsAndSmaller() == time
          && // FUTURE: could probably relax this to "same day" if ever needed
          (x.ReceivedCoinType == coinType || x.PaymentCoinType == coinType)
          && (x.ReceivedCoinType != x.PaymentCoinType)
          && // whut? oh this is just paranoia
          (x.ReceivedCoinsPerPaymentCoinListPrice != 0)
          && // same here; paranoia, but this happens for unpopulated data
      ...
      )
      {
        ...
        if (otherCoinPriceUsd != null)
        {
          ...
          _logger.LogInformation(
            $"determined pricePerCoinUsd = {(newPrice?.ToString() ?? "null")} for {coinType} at {time} by inference from list price of related {otherCoinType} transaction"
          );
          return newPrice;
        }
      }
