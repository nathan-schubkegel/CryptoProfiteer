...
{
  // This bot watches the market for an hour to learn the repeatable highs/lows currently present in the market
  // and tries to buy/sell within that measured high/low range.
  public class WaveAmplitudeBot : ITradeBot
  {
    private readonly Decimal _initialUsd;
    private readonly ConfigResult _config;
    private readonly LinkedList<Candle> _candles = new LinkedList<Candle>();
    private readonly (Decimal High, Decimal Low)? _measuredRange;

    public WaveAmplitudeBot(string coinType, CandleGranularity granularity, Decimal initialUsd)
    {
      _initialUsd = initialUsd;
    ...
    {
      // if I'm holding more Coins than USD, then don't buy. (that means I'm watching to sell!)
    ...
    {
      _candles.AddLast(args.Candle);
      while (_candles.Count > 150) _candles.RemoveFirst();
      
      if (_measuredRange == null)
      {
        // scan backward from latest candle to learn cyclic highs/lows
      }
    }
  }
}