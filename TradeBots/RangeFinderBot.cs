...
namespace CryptoProfiteer.TradeBots
{
  ...
  public class RangeFinderBot : ITradeBot
  {
    ...
    public WantsToBuyResult WantsToBuy(WantsToBuyArgs args)
    {
      ...
      if (_targetPurchaseCoinPrice == null)
        return default;
      ...
      {
        return new WantsToBuyResult { UsdToSpend = args.Usd, Note = $"buying because price looks low" };
      }
    ...
    public WantsToSellResult WantsToSell(WantsToSellArgs args)
    {
      ...
      {
        return new WantsToSellResult { CoinCountToSell = args.CoinCount, Note = $"selling because price looks high" };
      }
    ...
    public void ApplyNextCandle(NextCandleArgs args)
    {
      ...
      if (_candles.Count < _countOfCandlesToMeasure)
      {
        ...
        if (_candles.Count == _countOfCandlesToMeasure)
        {
          ...
          _lossPreventionSellCoinPrice =
            _targetSellCoinPrice
            - ((_targetPurchaseCoinPrice.Value - _targetSellCoinPrice) * _percentOfMeasuredRangeToGetOut);
        }
      }
    }
  }
